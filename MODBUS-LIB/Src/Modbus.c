...
uint8_t RingGetNBytes(modbusRingBuffer_t *xRingBuffer, uint8_t *buffer,
	...
	for (uCounter = 0;
			uCounter < uNumber && uCounter < xRingBuffer->u8available;
			uCounter++) {
...
void ModbusInit(modbusHandler_t *modH) {
	...
	if (numberHandlers < MAX_M_HANDLERS) {
		...
		if (modH->uModbusType == MB_SLAVE
				|| modH->uModbusType == MB_SLAVE_GATE) {
			//Create Modbus task slave
#if ENABLE_TCP == 1
			if (modH->xTypeHW == TCP_HW) {
				modH->myTaskModbusAHandle = osThreadNew(StartTaskModbusSlave,
						modH, &myTaskModbusA_attributesTCP);
			} else {
				modH->myTaskModbusAHandle = osThreadNew(StartTaskModbusSlave,
						modH, &myTaskModbusA_attributes);
			}
#else
		  modH->myTaskModbusAHandle = osThreadNew(StartTaskModbusSlave, modH, &myTaskModbusA_attributes);
				...
				|| modH->uModbusType == MB_MASTER_GATE) {
			//Create Modbus task Master  and Queue for telegrams

#if ENABLE_TCP == 1
			if (modH->xTypeHW == TCP_HW) {
				modH->myTaskModbusAHandle = osThreadNew(StartTaskModbusMaster,
						modH, &myTaskModbusB_attributesTCP);
			} else {
				modH->myTaskModbusAHandle = osThreadNew(StartTaskModbusMaster,
						modH, &myTaskModbusB_attributes);
			}
#else
		  modH->myTaskModbusAHandle = osThreadNew(StartTaskModbusMaster, modH, &myTaskModbusB_attributes);
			...
			modH->QueueTelegramHandle = osMessageQueueNew(MAX_TELEGRAMS,
					sizeof(modbus_t), &QueueTelegram_attributes);
	...
	for (i = 0; i < numberHandlers; i++) {
		...
		if ((TimerHandle_t*) mHandlers[i]->xTimerT35 == pxTimer) {
			if (mHandlers[i]->uModbusType == MB_MASTER
					|| mHandlers[i]->uModbusType == MB_MASTER_GATE) {
				xTimerStop(mHandlers[i]->xTimerTimeout, 0);
			}
			xTaskNotify(mHandlers[i]->myTaskModbusAHandle, 0,
...
void vTimerCallbackTimeout(TimerHandle_t *pxTimer) {
	...
	for (i = 0; i < numberHandlers; i++) {
		...
		if ((TimerHandle_t*) mHandlers[i]->xTimerTimeout == pxTimer) {
			xTaskNotify(mHandlers[i]->myTaskModbusAHandle, ERR_TIME_OUT,
					eSetValueWithOverwrite);
		}
			...
			if (buflen > 11) // minimum frame size for modbus TCP
				...
				if (buf[2] == 0 || buf[3] == 0) //validate protocol ID
					...
					if (uLength < (MAX_BUFFER - 2) && (uLength + 6) <= buflen) {
						...
						modH->u16TransactionID = (buf[0] << 8 & 0xff00)
								| buf[1];
...
void StartTaskModbusSlave(void *argument) {
	...
	for (;;) {
	  ...
	  if(modH-> xTypeHW == USB_CDC_HW)
	  {
		...
		if (modH->uModbusType == MB_SLAVE_GATE && modH->xTypeHW == TCP_HW) { //���� ����������� ������� �������, ��� ���� �������� �� ������ TCP
		...
		//������� ����� modH � ������� ����� ����������, ��� ����� ������� �� ������ �������
			...
			case MB_FC_READ_REGISTERS:
				telegram.u16RegAdd = word(modH->u8Buffer[ADD_HI],
						modH->u8Buffer[ADD_LO]); // start address in slave
				telegram.u16CoilsNo = word(modH->u8Buffer[NB_HI],
						modH->u8Buffer[NB_LO]); // number of elements (coils or registers) to read
				break;
			default:
			...
			if (modH->u8Buffer[ID] >= 1 && modH->u8Buffer[ID] <= 5) {
				...
				case 2:
					modH_gate = &ModbusH2;
					telegram.u16reg = ModbusDATA2;
					break;
				case 3:
					modH_gate = &ModbusH3;
					telegram.u16reg = ModbusDATA3;
					break;
				case 4:
					modH_gate = &ModbusH4;
					telegram.u16reg = ModbusDATA4;
					break;
				case 5:
					modH_gate = &ModbusH5;
					telegram.u16reg = ModbusDATA5;
					break;
				}
			} else {
				xSemaphoreGive(modH->ModBusSphrHandle); //Release the semaphore
...
//<s>���� ��� ������ � �� ����������, �� ���� ���������� modH_gate->u16TransactionID �� ��������, ����� ����� ��������� � ������</s>
//������ ��� ������, ��� ��� ��� ������� telegram
//������� � ��������� telegram ���������� u16TransactionID �������

			telegram.u16TransactionID = modH->u16TransactionID;
//� modH->newconnIndex ��������� ����� �� ������� ��������� ���� ���� ��������� �����
//��� ���� �������� � telegram->u8clientID ������� ������� �� MASTER
...
int8_t SendQuery(modbusHandler_t *modH, modbus_t telegram) {
	...
	case MB_FC_WRITE_MULTIPLE_COILS: // TODO: implement "sending coils"
		...
		for (uint16_t i = 0; i < u8bytesno; i++) {
			if (i % 2) {
				modH->u8Buffer[modH->u8BufferSize] = lowByte(
						telegram.u16reg[i / 2]);
			} else {
				modH->u8Buffer[modH->u8BufferSize] = highByte(
						telegram.u16reg[i / 2]);
...
static mb_errot_t TCPconnectserver(modbusHandler_t *modH, modbus_t *telegram) {
	...
	if (clientconn->conn == NULL) {
		...
		err = netconn_connect(clientconn->conn,
				(ip_addr_t*) &telegram->xIpAddress, telegram->u16Port);
     ...
     ulNotificationValue = ulTaskNotifyTake(pdTRUE, portMAX_DELAY);
      ...
      getRxBuffer(modH);
		...
		if (modH->uModbusType == MB_MASTER_GATE) {
			...
			//������� ����� modH � ������� ����� ���������� �����, � telegram ���� �������������� ������
			modbusHandler_t *modH_gate = xSemaphoreTake(
					modH_gate->ModBusSphrHandle, portMAX_DELAY); //before processing the message get the semaphore
			...
			if (modH->i8lastError == 0) // no error the error_OK, we need to use a different value than 0 to detect the timeout
				...
				xSemaphoreGive(modH->ModBusSphrHandle); //Release the semaphore
				xTaskNotify((TaskHandle_t )telegram.u32CurrentTask,
						ERR_OK_QUERY, eSetValueWithOverwrite);
			}
...
void get_FC1(modbusHandler_t *modH) {
	...
	for (i = 0; i < modH->u8Buffer[2]; i++) {
		...
		if (i % 2) {
			modH->u16regs[i / 2] = word(modH->u8Buffer[i + u8byte],
					lowByte(modH->u16regs[i / 2]));
		} else {

			modH->u16regs[i / 2] = word(highByte(modH->u16regs[i / 2]),
					modH->u8Buffer[i + u8byte]);
		}
...
void get_FC3(modbusHandler_t *modH) {
	...
	for (i = 0; i < modH->u8Buffer[2] / 2; i++) {
		modH->u16regs[i] = word(modH->u8Buffer[u8byte],
				modH->u8Buffer[u8byte + 1]);
		u8byte += 2;
	}
}
...
static void sendTxBuffer(modbusHandler_t *modH) {
    ...
    else if(modH->xTypeHW == USB_CDC_HW)
	{
		...
		err = netconn_write_vectors_partly(
				modH->newconns[modH->newconnIndex].conn, xNetVectors, 2,
...
int8_t process_FC1(modbusHandler_t *modH) {
	...
	uint16_t u16StartCoil = word(modH->u8Buffer[ADD_HI],
			modH->u8Buffer[ADD_LO]);
